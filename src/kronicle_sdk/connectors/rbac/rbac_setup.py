...
class KronicleRbac(KronicleUsrLogin):
    ...
    def patch_user(self, *, user_id: UUID, user: KronicleUser) -> KronicleUser:
        usr = self.patch(route=f"/users/{user_id}", body=user.model_dump())
        return KronicleUser(**usr)
    ...
    def patch_group(self, *, group_id: UUID, group: KronicleGroup) -> KronicleGroup:
        res = self.patch(route=f"/groups/{group_id}", body=group.model_dump())
        return KronicleGroup(**res)
    ...
    def patch_role(self, *, role_id: UUID, role: KronicleRole) -> KronicleRole:
        res = self.patch(route=f"/roles/{role_id}", body=role.model_dump())
        return KronicleRole(**res)

    def delete_role(self, *, role_id: UUID) -> KronicleRole | None:
        res = self.delete(route=f"/roles/{role_id}")
        return KronicleRole(**res) if res else None

    def assign_role_to_user(self, *, role_id: UUID, user_id: UUID) -> dict:
        return self.post(route=f"/roles/{role_id}/users?user_id={user_id}")

    def assign_role_to_group(self, *, role_id: UUID, group_id: UUID) -> dict:
        return self.post(route=f"/roles/{role_id}/groups?group_id={group_id}")

    def remove_role_from_user(self, *, role_id: UUID, user_id: UUID) -> dict:
        return self.delete(route=f"/roles/{role_id}/users/{user_id}")

    def remove_role_from_group(self, *, role_id: UUID, group_id: UUID) -> dict:
        return self.delete(route=f"/roles/{role_id}/groups/{group_id}")
